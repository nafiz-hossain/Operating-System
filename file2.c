...
int sys_open(userptr_t filename, int flags, int *ret) 
{
	...
	if(kfilename == NULL)return ENFILE;
	...
	for(;i<MAX_PROCESS_OPEN_FILES;i++)
		if(curproc->file_table[i] == NULL)break;
	...
	if(open_file_count>=MAX_SYSTEM_OPEN_FILES)
	{
		kfree(kfilename);
		return ENFILE;
	}
...
int sys_write(int filehandler, userptr_t buf, size_t size, int *ret)
{
	...
	uio_uinit(&file_iovec, &file_uio, buf, size, file->offset, UIO_WRITE);
	int result = VOP_WRITE(file->v_ptr, &file_uio);
	...
	file->offset = file_uio.uio_offset;
...
int sys_read(int filehandler, userptr_t buf, size_t size, int *ret)
{
	...
	lock_acquire(file->flock);
	off_t old_offset = file->offset;
	uio_uinit(&file_iovec, &file_uio, buf, size, file->offset, UIO_READ);
	int result = VOP_READ(file->v_ptr, &file_uio);
...
int sys_lseek(int fd, off_t pos, userptr_t whence_ptr, int *ret)
{
	...
	if(!VOP_ISSEEKABLE(file->v_ptr))return ESPIPE;
	...
	{
		if(stats.st_size+pos<0)return EINVAL;
		lock_acquire(file->flock);
